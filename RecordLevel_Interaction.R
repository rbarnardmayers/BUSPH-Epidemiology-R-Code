...
# Crude RR ----
output_obs_c <- glm(d~e, data=misc2[misc2$iteration ==1,], family=binomial(link='logit')) 
...
estim1 <- data.frame(Parameter="e_exp_t", e_conv_a = output_obs_a$coefficients[2],
                     ...
                     interaction_conv_a = output_obs_a$coefficients[7] + output_obs_a$coefficients[2],
                     interaction_stderr_a = get.SE(getCI.int(output_obs_a, 2, 7)))

# Obtain RR estimates for each iteration----
library(tidyr)
misc3 <- misc2[!is.na(misc2$e_exp_t), ]

estim2 <- misc3 %>% 
  group_by(iteration) %>% 
  do(model = glm(d ~ e_exp_t * GestDiab + Age + PregnancyWeight_Gain + 
                   Prepregnancy_Weight , data = ., 
                 family = binomial(link = "logit"))) %>% 
  mutate(e_syst = as.numeric(tidy(model)[2,2]),
         stderr_syst = as.numeric(tidy(model)[2,3]),
         interaction_syst = as.numeric(tidy(model)[7,2] + tidy(model)[2,2]), 
         interaction_stderr = get.SE(getCI.int(model, 2, 7))) 

# estim2 <- estim2 %>% unnest(cols = output) %>% 
#   gather("logOR", "value", -1)  %>%
#    mutate(measurement = rep(rep(c("e_syst", "interaction_syst"), 2), length(estim2$iteration)))  %>% 
#   mutate(title = paste0(measurement, "_", logOR))  
# 
# estim2 <- estim2[c("iteration", "value", "title")] %>%
# spread("title", "value")
# write.csv(estim2, "estim2.csv")
# colnames(estim2) <- c("iteration", "e_syst", "stderr_syst","interaction_syst" ,"interaction_stderr")
estim2 <- estim2[ , !(names(estim2) %in% c("model"))]
estim2$Parameter <- rep("e_exp_t", nrow(estim2))
